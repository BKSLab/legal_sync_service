...
Сквозная история от постановки на контроль до обновления коллекции:
[прослеживаемость Legal Sync ↔ RAG](docs/TRACEABILITY.md).
...
| Документация (`/admin/documentation`) | Этот README с оглавлением, таблицами, подсветкой кода и схемами |

«Документация» читает **этот же `README.md` из корня приложения**, без отдельной
копии текста или записей в БД. После изменения файла следующее открытие страницы
показывает новую версию. В Docker отображается README из собранного образа:
после `git pull` нужно пересобрать и пересоздать приложение обычной командой
обновления. Страница и скачивание исходного Markdown доступны после входа.
Кнопки копирования работают и по HTTP; ссылки на файлы проекта открывают GitHub.
Схемы Mermaid строятся локальной библиотекой из состава приложения, без CDN.
